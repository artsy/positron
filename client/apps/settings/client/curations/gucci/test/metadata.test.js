...
describe('Metadata', () => {
  ...
  it('renders saved data', () => {
    ...
    expect(component.find('input').at(0).props().defaultValue).toBe('What Happened in the Past?')
    expect(component.find('textarea').at(0).props().defaultValue).toBe('General Description')
    expect(component.find('textarea').at(1).props().defaultValue).toBe('A series of films optimized for social media')
    expect(component.find('input').at(1).props().defaultValue).toBe('Good Morning, What Happened?')
    expect(component.find('input').at(2).props().defaultValue).toBe('Artsy Editors')
    expect(component.find('input').at(3).props().defaultValue).toBe('magazine,article,gucci')
    expect(component.find('input').at(4).props().defaultValue).toBe('women,gender,equality')
  })
  ...
  it('Updates section metadata inputs', () => {
    ...
    textarea.at(1).simulate('change', { target: { value: 'Social Description' } })
