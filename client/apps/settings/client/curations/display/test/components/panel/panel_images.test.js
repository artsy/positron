...
describe('PanelImages', () => {
  ...
  it('renders all fields', () => {
    ...
    expect(component.find('label').at(0).text()).toMatch('Image / Video')
    expect(component.find('label').at(1).text()).toMatch('Logo')
  })
  ...
  it('renders cover image field if asset is video', () => {
    ...
    expect(component.find('label').at(1).text()).toMatch('Cover Image')
    expect(component.find(ImageUpload).at(1).props().name).toMatch('panel.cover_img_url')
  })
  ...
  it('Calls props.onChange on image change', () => {
    ...
    expect(props.onChange.mock.calls[0][1][0].url).toMatch('http://new-image.jpg')
