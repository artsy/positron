...
import { ErrorMessage } from "client/typings"
import {
  ArtworkGridArtwork,
  ArtworkGridColumns,
  SectionData,
...
import { ArtworkInputs, RadioInput } from "../../images/components/controls"
...
import {
  COLUMN_OPTIONS,
  DEFAULT_COLUMNS,
  OnChangeArtworkGridSection,
} from "../constants"

interface Props {
  editSection: SectionData
  logErrorAction: (error: ErrorMessage) => void
...
export class ArtworkGridControls extends Component<Props> {
  ...
  onSelectArtworks = (artworks: Array<ArtworkGridArtwork | null>) => {
    ...
    const valid = artworks.filter(
      (artwork): artwork is ArtworkGridArtwork =>
        !!artwork && artwork.type === "artwork"
    )
          ...
          <Flex alignItems="center">
            ...
            <Flex pl={2}>
              {COLUMN_OPTIONS.map(option => (
                <Flex key={option} alignItems="center" pr={2}>
                  <RadioInput
                    data-columns={option}
                    isActive={columns === option}
                    onClick={() => this.setColumns(option)}
                  />
                  <FormLabel color="white">{option}</FormLabel>
                </Flex>
              ))}
            </Flex>
          </Flex>
