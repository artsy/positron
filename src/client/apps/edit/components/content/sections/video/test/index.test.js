...
describe('Video', () => {
  ...
  it('Renders a saved video and cover image', () => {
    ...
    expect(component.find(SectionVideo).props().section.cover_image_url).toMatch(video.cover_image_url)
  ...
  it('#onProgress sets state.progress, renders progress bar if state.progress exists', () => {
    ...
    const progress = 0.5
    const component = getWrapper(props)
    component.find(SectionVideo).instance().onProgress(progress)
    component.update()

    expect(component.find(SectionVideo).instance().state.progress).toEqual(progress)
    expect(component.find(ProgressBar).exists()).toBe(true)
    expect(component.find(ProgressBar).getElement().props.progress).toBe(progress)
  })
  ...
  describe('Caption', () => {
    ...
    it('Renders a saved caption', () => {
      props.editing = true
      const component = getWrapper(props)
      const caption = props.section.caption.replace('<p>', '').replace('</p>', '')
    ...
    it('Can edit a caption', () => {
      ...
      component.find(Paragraph).getElement().props.onChange(caption)
  ...
  describe('Remove Button', () => {
    ...
    it('Can remove the cover_image_url', () => {
      ...
      component.find(RemoveButton).simulate('click')
      expect(props.onChangeSectionAction.mock.calls[0][0]).toBe('cover_image_url')
      expect(props.onChangeSectionAction.mock.calls[0][1]).toBeFalsy()
    })
  })
