...
describe("Draft Utils: Text Selection", () => {
  ...
  it("#divideEditorState Divides an editorState into two, returns html for new section", () => {
    ...
    const newStateToHtml = dividedState.currentSectionState
      .getCurrentContent()
      .getPlainText()
    ...
    expect(newStateToHtml).not.toMatch(
      "So what would de Gébelin’s reaction be?"
    )
    expect(dividedState.newSection).toMatch(
      "So what would de Gébelin’s reaction be?"
    )
  })
  ...
  it("#addLinkToState Applies a new link entity to editorState at selection", () => {
    ...
    const stateWithLink = addLinkToState(
