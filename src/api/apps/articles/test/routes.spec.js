...
describe('routes', function() {
  ...
  beforeEach(function() {
    ...
    for (let method of Array.from((this.methods = ['where', 'save', 'destroy', 'find']))) {
  ...
  afterEach(function() {
    return Array.from(this.methods).map((method) => this.Article[method].restore());
  });
  ...
  describe('#index', function() {
    ...
    it('sends a list of published articles by author', function() {
      ...
      return this.res.send.args[0][0].results[0].title.should.containEql('Top Ten');
    });
    ...
    it('returns an error if channel_id is not provided for unpublished', function() {
      ...
      return this.res.err.args[0][1].should.containEql('Must pass channel_id to view unpublished articles');
    });
    ...
    it('denies unpublished articles to non channel members', function() {
      ...
      routes.index(this.req, this.res, this.next);
      this.res.err.args[0][0].should.equal(401);
      return this.res.err.args[0][1].should.containEql('Must be a member of this channel');
    });
  ...
  describe('#create', () => it('creates an article with data', function() {
    this.User.hasChannelAccess = sinon.stub().returns(true);
  ...
  describe('#update', function() {
    ...
    return it('defaults to the logged in user for author_id', function() {
      ...
      this.req.article = fixtures().articles;
      routes.create(this.req, this.res);
      return this.Article.save.args[0][0].author_id.should.equal(this.req.user._id.toString());
    });
  });

  describe('#delete', () => it('deletes an existing article', function() {
    this.req.article = fixtures().articles;
  ...
  return describe('#find', function() {
    ...
    it('looks for an article and attaches it to req', function() {
      ...
      this.Article.find.args[0][1](null, _.extend(fixtures().articles, {
