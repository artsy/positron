    ...
    it("can get a list of published articles without a logged in user", done =>
      fabricate(
        ...
        (err, _articles) => {
          ...
          request
            .get(`http://localhost:${port}/articles?count=true`)
            .end((error, res) => {
              if (error) {
                done(error)
              }
    ...
    xit("does not allow viewing drafts", done =>
      fabricate(
        ...
        (err, _articles) => {
          ...
          request
            .get(`http://localhost:${port}/articles/5086df098523e60002000012`)
            .end((error, _res) => {
              if (error) {
                done(error)
              }
    ...
    it("gets a list of articles by author", done => {
      fabricate(
        ...
        (err, _articles) => {
          ...
          request
            .get(
              `http://localhost:${port}/articles?author_id=${
                user._id
    ...
    it("gets a list of articles by channel", done => {
      fabricate(
        ...
        (err, _articles) => {
          ...
          request
            .get(
              `http://localhost:${port}/articles?channel_id=5086df098523e60002000012&published=true&count=true`
            )
            .set({ "X-Access-Token": token })
            .end((error, res) => {
              if (error) {
    ...
    it("denies unpublished requests", done =>
      fabricate(
        ...
        (err, _articles) => {
          ...
          request
            .get(`http://localhost:${port}/articles?published=false`)
            .end((error, res) => {
    ...
    it("updates an article", done => {
      fabricate(
        ...
        (err, _articles) => {
          ...
          request
            .put(`http://localhost:${port}/articles/5086df098523e60002000012`)
            .send({
              title: "Hellow Wrld",
              author_id: "5086df098523e60002000012",
    ...
    it("deletes an article", done => {
      fabricate(
        ...
        (err, _articles) => {
          ...
          request
            .del(`http://localhost:${port}/articles/5086df098523e60002000012`)
            .set({ "X-Access-Token": token })
            .end((error, _res) => {
              if (error) {
                done(error)
