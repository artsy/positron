...
describe('Save', function() {
  ...
  beforeEach(function(done) {
    ...
    Save.__set__('request', { post: (this.post = sinon.stub()).returns({
      send: (this.send = sinon.stub()).returns({
        end: sinon.stub().yields()})
    })
  }
    );
    Save.__set__('distributeArticle', (this.distributeArticle = sinon.stub().yields()));
    Save.__set__('deleteArticleFromSailthru', (this.deleteArticleFromSailthru = sinon.stub().yields()));
  ...
  describe('#removeStopWords', function() {
    ...
    it('removes stop words from a string', function(done) {
      this.removeStopWords('Why. the Internet Is Obsessed with These Videos of People Making Things').should.containEql('internet obsessed videos people making things');
      this.removeStopWords('Heirs of Major Jewish Art Dealer Sue Bavaria over $20 Million of Nazi-Looted Art').should.containEql('heirs major jewish art dealer sue bavaria 20 million nazi-looted art');
      this.removeStopWords('Helen Marten Wins UK’s Biggest Art Prize—and the 9 Other Biggest News Stories This Week').should.containEql('helen marten wins uks art prize 9 news stories week');
      return done();
    });
  ...
  describe('#onPublish', function(done) {
    ...
    it('generates slugs and published_at if not present', done => Save.onPublish({
    ...
    return it('does not generate published_at if scheduled', done => Save.onPublish({
      thumbnail_title: 'a title',
  ...
  describe('#generateSlugs', function() {
    ...
    it('generates a slug', done => Save.generateSlugs({
    ...
    it('appends a date to the slug if it exists in the database already', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      author: { name: 'Molly'
    },
    ...
    it('does not append a date to the slug if it exists in the slugs array for that article already', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      slugs: ['molly-clockwork'],
      author: { name: 'Molly'
    ...
    it('appends the current date to the slug if the slug exists in the database already but the date is invalid', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      author: { name: 'Molly'
    },
    ...
    it('appends the current date to the slug if the slug exists in the database already but there is no publish date', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      author: { name: 'Molly'
    }
    ...
    it('moves the slug to the end of the slugs array and does not append a date if the slug is present elsewhere in the array', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      author: { name: 'Molly'
    },
    ...
    it('does not append a date to the slug if a slug with that date exists in the slugs array for that article already', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      author: { name: 'Molly'
    },
    ...
    it('it moves the slug with the appended date to the end of the slugs array if a slug with that date exists in the slugs array for that article already', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      author: { name: 'Molly'
    },
    ...
    it('appends unix to the slug if it exists already and it is a draft', done => Save.sanitizeAndSave( () => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: false,
      author: { name: 'Molly'
    },
    ...
    it('appends unix timestamp for current date to the slug if the slug exists already and it is a draft and there is no publish date', function(done) {
      ...
      return Save.sanitizeAndSave( () => Save.generateSlugs({
        thumbnail_title: 'Clockwork',
        published: false,
        author: { name: 'Molly'
      }
    ...
    return it('does not append author if it is series layout', done => Save.generateSlugs({
      thumbnail_title: 'Clockwork',
      published: true,
      author: { name: 'Molly'
    },
  ...
  describe('#onUnpublish', function() {
    ...
    it('generates slugs and deletes article from sailthru', function(done) {
      return Save.onUnpublish({
        ...
        article.slugs.length.should.equal(1);
        this.deleteArticleFromSailthru.args[0][0].should.containEql('video/artsy-editorial-delete-title');
        return done();
      });
    });
    ...
    return it('Regenerates the slug with stop words removed', function(done) {
      return Save.onUnpublish({
        ...
        article.slugs.length.should.equal(1);
        this.deleteArticleFromSailthru.args[0][0].should.containEql('article/artsy-editorial-one-new-york-building-changed-way-art-made-seen-sold');
        return done();
      });
    });
  });
  ...
  return describe('#sanitizeAndSave', function() {
    ...
    it('skips sanitizing links that do not have an href', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    it('can save follow artist links (whitelist data-id)', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    it('can save layouts on text sections', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    it('saves email metadata', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    it('does not override email metadata', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    it('saves generated descriptions', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    it('does not override description', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    it('Strips linebreaks from titles', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
    ...
    return it('saves media', done => Save.sanitizeAndSave( () => Article.find('5086df098523e60002000011', function(err, article) {
