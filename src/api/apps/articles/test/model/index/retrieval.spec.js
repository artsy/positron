...
describe('Article Retrieval', function() {
  ...
  describe('#where', function() {
    ...
    it('can return all articles along with total and counts', done => Article.where({ count: true }, function(err, res) {
    ...
    it('can return all articles along with total and counts', done => Article.where({}, function(err, res) {
      const { total, count, results } = res;
      if (total != null) {
    ...
    it('can return articles by an author', function(done) {
      ...
      return fabricate('articles', {
        ...
        title: 'Hello Wurld'
      }, () => Article.where({ author_id: aid.toString(), count: true }, function(err, res) {
        const { total, count, results } = res;
    ...
    it('can return articles by published', done => fabricate('articles', _.times(3, () => ({
      published: false,
      title: 'Moo baz'
    })), () => Article.where({ published: false, count: true }, function(err, { total, count, results }) {
    ...
    it('can change skip and limit', done => fabricate('articles', [
      { title: 'Hello Wurld' },
      { title: 'Foo Baz' }
    ...
    it('sorts by -updated_at by default', done => fabricate('articles', [
    ...
    it('can find articles featured to an artist', done => fabricate('articles', [
      {
        title: 'Foo',
    ...
    it('can find articles featured to an artwork', done => fabricate('articles', [
      {
        title: 'Foo',
    ...
    it('can find articles sorted by an attr', done => db.articles.drop(() => fabricate('articles', [
    ...
    it('can find articles added to multiple fairs', done => fabricate('articles', [
    ...
    it('can find articles by a fair', done => fabricate('articles', [
      { title: 'C', fair_ids: [ObjectId('4dc98d149a96300001003033')] },
      { title: 'A', fair_ids: [ObjectId('4dc98d149a96300001003033')] }
    ], () => Article.where(
    ...
    it('can return articles by a fair programming id', function(done) {
      ...
      }, () => Article.where({ fair_programming_id: '52617c6c8b3b81f094000013', count: true }, function(err, res) {
        const { total, count, results } = res;
        total.should.equal(11);
        count.should.equal(1);
        results[0].title.should.equal('Hello Wurld');
    ...
    it('can return articles by a fair artsy id', function(done) {
      ...
      }, () => Article.where({ fair_artsy_id: '53da550a726169083c0a0700', count: true }, function(err, res) {
        const { total, count, results } = res;
        total.should.equal(11);
        count.should.equal(1);
        results[0].title.should.equal('Hello Wurld');
    ...
    it('can return articles by a fair about id', function(done) {
      ...
      }, () => Article.where({ fair_about_id: '53da550a726169083c0a0700', count: true }, function(err, res) {
        const { total, count, results } = res;
        total.should.equal(11);
        count.should.equal(1);
        results[0].title.should.equal('Hello Wurld');
    ...
    it('can find articles added to a partner', done => fabricate('articles', [
    ...
    it('can find articles by query', done => fabricate('articles', [
      { thumbnail_title: 'Foo' },
      { thumbnail_title: 'Bar' },
      { thumbnail_title: 'Baz' }
    ], () => Article.where(
    ...
    it('can find articles by all_by_author', done => fabricate('articles', [
      {
    ...
    ], () => Article.where({ all_by_author: '4d8cd73191a5c50ce220002b', count: true }, function(err, res) {
      const { total, count, results } = res;
    ...
    it('can find articles by super article', done => fabricate('articles', [
      {
        author_id: ObjectId('4d8cd73191a5c50ce220002a'),
        title: 'Hello Wurld',
    ...
    ], () => Article.where({ is_super_article: true, count: true }, function(err, res) {
      const { total, count, results } = res;
      total.should.equal(13);
      count.should.equal(1);
    ...
    it('can find super article by article (opposite of the above test!)', function(done) {
      ...
      ], () => Article.where({ super_article_for: childArticleId.toString(), count: true }, function(err, res) {
        const { total, count, results } = res;
        total.should.equal(12);
        count.should.equal(1);
    ...
    it('can find articles by tags', done => fabricate('articles', [
      {
        author_id: ObjectId('4d8cd73191a5c50ce220002a'),
        tags: [ 'pickle', 'samosa' ],
    ...
    ], () => Article.where({ tags: ['pickle', 'muffin'], count: true }, function(err, res) {
      const { total, count, results } = res;
    ...
    it('can find articles by tracking_tags', done => fabricate('articles', [
      {
        author_id: ObjectId('4d8cd73191a5c50ce220002a'),
    ...
    ], () => Article.where({ tracking_tags: ['video', 'evergreen'], count: true }, function(err, res) {
      const { total, count, results } = res;
      total.should.equal(14);
      count.should.equal(3);
    ...
    it('can find articles by artist biography', done => fabricate('articles', [
      {
        title: 'Hello Wurld',
        published: true,
    ...
    it('can find articles by channel_id', done => fabricate('articles', [
      {
        title: 'Hello Wurld',
        published: true,
    ...
    it('can find articles by partner_channel_id', done => fabricate('articles', [
      {
        title: 'Hello Wurld',
        published: true,
    ...
    it('can find articles by scheduled', done => fabricate('articles', [
      {
        title: 'Hello Wurld',
        published: false,
    ...
    it('can return articles by indexable', done => fabricate('articles', _.times(3, () => ({
      indexable: false,
      title: 'Moo baz'
    })), () => Article.where({ indexable: true, count: true }, function(err, { total, count, results }) {
      total.should.equal(13);
      count.should.equal(10);
    ...
    it('can return articles by not indexable', done => fabricate('articles', _.times(3, () => ({
      indexable: false,
      title: 'Moo baz'
    })), () => Article.where({ indexable: false, count: true }, function(err, { total, count, results }) {
      total.should.equal(13);
      count.should.equal(3);
      results[0].title.should.equal('Moo baz');
      return done();
    ...
    return it('can omit articles', done => fabricate('articles', [
      {
        title: 'Hello Wurld',
        _id: ObjectId('5086df098523e60002000018'),
        published: true
      }
    ], () => Article.where({ omit: ['5086df098523e60002000018'], count: true }, function(err, { total, count, results }) {
      total.should.equal(11);
      count.should.equal(10);
      return done();
    })));
  ...
  return describe('#find', function() {
    ...
    it('finds an article by an id string', done => fabricate('articles', { _id: ObjectId('5086df098523e60002000018') }, () => Article.find('5086df098523e60002000018', function(err, article) {
      article._id.toString().should.equal('5086df098523e60002000018');
      return done();
    })));

    return it('can lookup an article by slug', done => fabricate('articles', {
      _id: ObjectId('5086df098523e60002000018'),
      slugs: ['foo-bar']
